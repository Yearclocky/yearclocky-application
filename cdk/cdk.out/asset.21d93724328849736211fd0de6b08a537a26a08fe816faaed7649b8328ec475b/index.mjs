...
var tasksStore = /* @__PURE__ */ new Map();
seedData();
...
function seedData() {
  ...
  tasksStore.set(seededTask.id, seededTask);
}

// packages/app/src/routes/tasks.ts
function tasks(method, segments, rawBody) {
  if (segments.length === 1) {
    if (method === "GET") {
      return ok({ items: [...tasksStore.values()] });
    }
    if (method === "POST") {
      const payload = parseJsonBody(rawBody);
      return created(createTask(payload));
    }
  }
  if (segments.length === 2) {
    ...
    if (method === "PATCH") {
      const payload = parseJsonBody(rawBody);
      return ok(updateTask(taskId, payload));
    }
    if (method === "DELETE") {
      deleteTask(taskId);
      return noContent();
...
// packages/app/src/routes/users.ts
function users2(method, segments, rawBody) {
  if (segments.length === 1) {
    if (method === "GET") {
    ...
    if (method === "POST") {
      const payload = parseJsonBody(rawBody);
      return created(createUser(payload));
    }
  }
  if (segments.length === 2) {
    ...
    if (method === "PATCH") {
      const payload = parseJsonBody(rawBody);
      return ok(updateUser(userId, payload));
    }
    if (method === "DELETE") {
      deleteUser(userId);
      return noContent();
...
function deleteUser(userId) {
  ...
  const hasTasks = [...tasksStore.values()].some((task) => task.userId === userId);
  if (hasTasks) {
    throw new HttpError(
      409,
      "USER_HAS_TASKS",
...
// packages/app/src/routes/yearclocks.ts
function yearclocks2(method, segments, rawBody) {
  if (segments.length === 1) {
    if (method === "GET") {
    ...
    if (method === "POST") {
      const payload = parseJsonBody(rawBody);
      return created(createYearclock(payload));
    }
  }
  if (segments.length === 2) {
    ...
    if (method === "PATCH") {
      const payload = parseJsonBody(rawBody);
      return ok(updateYearclock(yearclockId, payload));
    }
    if (method === "DELETE") {
...
function deleteYearclock(yearclockId) {
  ...
  const hasTasks = [...tasksStore.values()].some(
...
async function handler(event) {
  try {
    ...
    switch (resource) {
      case "tasks":
        return tasks(method, segments, event.body);
      case "yearclocks":
        return yearclocks2(method, segments, event.body);
      case "users":
        return users2(method, segments, event.body);
      default:
